...
STRATEGY_CONFIG = {
    ...
    # 核心策略参数 (根据之前的最优解填写)
    'trailing_atr':  2.0,   # 止盈宽松度
    'buy_conf':      0.65,  # 买入信心门槛
    'target_up':     2.0,   # [训练] 贪婪度
    'risk_trig':     0.5,   # [训练] 胆量/风控敏感度
    
    # 固定参数
    'stop_down':     1.0,   # [固定] 容忍度
    'lookback_days': 600,   # 数据回溯长度
    'train_min':     300,   # 最小训练天数
...
# ==============================================================================
MY_PORTFOLIO = {
    'total_capital':   50000.0,  # 总投入本金 (比如10万)
    'current_cash':    50000.0,   # 当前账户里的可用现金
    'hold_shares':     0,     # 当前持仓股数 (如果没有填0)
    'avg_cost':        0,     # 当前持仓成本价 (如果没有填0)
    'highest_price':   0,     # 持仓期间见过的最高价 (用于移动止盈，若刚买填当前价)
    'units_used':      0          # 已使用的子弹数 (0, 1, 2, 3)
}
