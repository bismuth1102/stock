...
def make_decision(strategy_key):
    cfg = STRATEGIES[strategy_key]
    pf = cfg['portfolio']
    
    print(f"\n🚀 正在分析 {cfg['name']} ({cfg['code']})...")
    ...
    if pf['hold_shares'] > 0:
        ...
        if pnl_pct <= -0.10:
            ...
            sell_trigger = True
            reason = "弱止损(-5%)退弹"
        elif (not trend_ok) or (sell_prob > 0.85):
            sell_trigger = True
            reason = f"风控撤退 (趋势:{trend_ok}, AI险:{sell_prob:.2f})"
        elif (pf['highest_price'] > pf['avg_cost'] * 1.08) and (last_row['最低'] <= trailing_price):
            sell_trigger = True
            reason = f"移动止盈 (破{trailing_price:.3f})"
    ...
    if pf['units_used'] < cfg['max_bullets']:
        ...
        if pf['units_used'] == 0:
            ...
            if trend_ok and (buy_prob > cfg['buy_conf']) and risk_pass and (close > pf['avg_cost']):
                buy_signal = True
                reason = f"加仓 (第{pf['units_used']+1}发)"
